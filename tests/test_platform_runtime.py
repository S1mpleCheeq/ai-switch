...
from ai_switch.platforms import runtime
...
class RuntimeTests(unittest.TestCase):
    ...
    def test_default_editor(self):
        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(
                runtime.editor_command(), ["notepad.exe" if os.name == "nt" else "vi"]
            )
    ...
    def test_hook_recognition_preserves_unrelated_and_shell_commands(self):
        ...
        self.assertTrue(
            runtime.is_managed_hook(
                shlex.join(["python3.10", "-X", "utf8", str(script)]), script
            )
        )
        self.assertFalse(
            runtime.is_managed_hook(
                shlex.join(["python3", str(script.with_name("unrelated.py"))]), script
            )
        )
        self.assertFalse(
            runtime.is_managed_hook(
                "echo hello && " + shlex.join(["python3", str(script)]), script
            )
        )
    ...
    def test_npm_shim_is_resolved_without_cmd_shell(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "node.exe").touch()
            self.assertEqual(
                runtime.native_command([str(shim), "resume", "a&b"]),
            ...
            (claude / "package.json").write_text(
                json.dumps({"bin": {"claude": "bin/claude.exe"}})
            )
            self.assertEqual(
                runtime.native_command([str(root / "claude.cmd"), "a&b"]),
