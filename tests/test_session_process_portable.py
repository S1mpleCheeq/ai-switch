...
from ai_switch.platforms import portable as process
...
class MalformedLockTests(unittest.TestCase):
    ...
    def test_fifo_is_rejected_without_hanging_or_signalling(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            os.mkfifo(path)
            script = (
                "import sys;from pathlib import Path;from ai_switch.platforms import portable as p\n"
...
@unittest.skipUnless(
    sys.platform == "darwin" or os.name == "nt", "Native macOS/Windows takeover"
)
class PortableProcessTests(unittest.TestCase):
    ...
    def holder(self, paths=None):
        child = subprocess.Popen(
            [sys.executable, "-u", "-c", HOLDER, *map(str, paths or [self.path])],
        ...
        def identity(pid):
            ...
            if pid == child.pid:
                result["executable"] = str(
                    self.home / ("codex.exe" if os.name == "nt" else "codex")
                )
    ...
    def test_default_and_dry_run_do_not_terminate(self):
        ...
        with (
            self.native(child),
            patch.object(
                process, "terminate", side_effect=AssertionError("no termination")
            ),
        ):
            self.assertTrue(
    ...
    def test_non_client_and_multi_session_processes_are_rejected(self):
        child = self.holder(
            [self.path, self.path.with_name(str(uuid.uuid4()) + ".lock")]
        )
        with self.assertRaisesRegex(process.TakeoverError, "不是当前用户"):
            process.ensure_available(self.home, self.session, takeover=True)
        with (
            self.native(child),
            self.assertRaisesRegex(process.TakeoverError, "其他会话"),
        ):
            process.ensure_available(self.home, self.session, takeover=True)
        self.assertIsNone(child.poll())
    ...
    def test_ambiguous_owner_and_identity_race_are_rejected(self):
        child = self.holder()
        with patch.object(
            process, "file_users", return_value={child.pid, child.pid + 100000}
        ):
        ...
        with (
            ...
            patch.object(
                process,
                "writer",
                side_effect=[process.writer(self.home, self.session), None],
            ),
        ):
