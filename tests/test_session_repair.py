...
import shutil
from ai_switch.sessions import repair
...
class SessionTests(unittest.TestCase):
    def setUp(self):
        ...
        atomic_write(
            ...
            s.json_bytes(
                dict(
                    version=1,
                    paths=dict(
                        codex=str(self.home / "config.toml"),
                        claude=str(self.root / "claude/settings.json"),
                    ),
                    active=dict(codex="micu", claude="micu"),
                    last_backup=None,
                )
            ),
        )
        ...
        self.db.executescript(
            "CREATE TABLE threads(id TEXT PRIMARY KEY, rollout_path TEXT, cwd TEXT, model_provider TEXT, title TEXT, name TEXT, source TEXT, thread_source TEXT, agent_path TEXT, archived INTEGER DEFAULT 0, updated_at INTEGER, updated_at_ms INTEGER); CREATE TABLE thread_spawn_edges(parent_thread_id TEXT, child_thread_id TEXT);"
        )
    ...
    def add_thread(
        ...
        path = (
            self.home
            / "sessions/2026/09/01"
            / f"rollout-2026-09-01T00-00-00-{ident}.jsonl"
        )
        atomic_write(
            path, b"".join(s.json_bytes(x).replace(b"\n", b"") + b"\n" for x in lines)
        )
        self.db.execute(
    ...
    def message(
        self,
        ident="item_old_message",
        text="Retain item_old_message inside text unchanged",
    ):
        return dict(
            type="response_item",
            payload=dict(
    ...
    def add_segment(self, ident, parent_path, cut, records, update=True):
        ...
        meta["history_base"] = dict(
            thread_id=repair.rollout_segment_id(parent_path),
            end_ordinal_exclusive=cut,
            end_byte_offset=offset,
        )
        path = parent_path.with_name(
            f"rollout-2026-09-23T00-00-00-{ident}_{uuid.uuid4()}.jsonl"
        )
        rows = [dict(type="session_meta", payload=meta)] + records
        for i, row in enumerate(rows):
            row["ordinal"] = cut + i
        atomic_write(path, repair.serialize(rows))
        if update:
            self.db.execute(
                "UPDATE threads SET rollout_path=? WHERE id=?", (str(path), ident)
            )
            self.db.commit()
        return path
    ...
    def test_list_hides_subagents_before_limit_and_retains_forks(self):
        ...
        fork, _ = self.add_thread(
            [],
            base=dict(thread_id=user, end_ordinal_exclusive=1, end_byte_offset=0),
            updated=11,
        )
        guardian, _ = self.add_thread(
            [],
            source='{"subagent":{"other":"guardian"}}',
            thread_source="guardian_review",
            updated=30,
        )
        sub, _ = self.add_thread(
            [],
            source='{"subagent":{"thread_spawn":{"depth":1}}}',
            thread_source="subagent",
            agent_path="/root/work",
            updated=29,
        )
        ...
        self.assertEqual(
            [x["id"] for x in self.m.sessions("codex", 2, True)], [guardian, sub]
        )
        self.assertNotIn("[subagent]", self.cli("sessions", "--app", "codex"))
        self.assertIn(
            "[subagent]", self.cli("sessions", "--app", "codex", "--include-subagents")
        )
    ...
    def test_list_detects_spawn_edges_and_keeps_root_agent_path(self):
        ...
        self.db.execute("INSERT INTO thread_spawn_edges VALUES(?,?)", (user, child))
        self.db.commit()
        self.assertEqual([x["id"] for x in self.m.sessions("codex")], [user])

    def test_list_older_schema_uses_source_and_updated_at(self):
        self.db.executescript(
            "DROP TABLE threads; CREATE TABLE threads(id TEXT,cwd TEXT,model_provider TEXT,title TEXT,source TEXT,archived INTEGER,updated_at INTEGER);"
        )
        self.db.executemany(
            "INSERT INTO threads VALUES(?,?,?,?,?,?,?)",
            [
                ("u", "/root", "micu", "User", "cli", 0, 1),
                ("s", "/root", "aster", "Subagent", '{"subagent":"review"}', 0, 2),
            ],
        )
        self.db.commit()
    ...
    def test_claude_sidechains_filtered_before_limit(self):
        ...
        (directory / (user + ".jsonl")).write_text(
            json.dumps(dict(cwd=str(self.root), isSidechain=False)) + "\n"
        )
        (directory / (sub + ".jsonl")).write_text(
            json.dumps(dict(cwd=str(self.root), isSidechain=True)) + "\n"
        )
    ...
    def test_repair_fork_cut_uses_ordinals_and_preserves_payloads(self):
        ...
        output = dict(
            ...
            payload=dict(
                ...
                id="fco_keep",
                call_id="call_keep",
                output="keep item_call",
            ),
        )
        parent, parent_path = self.add_thread(
            [
                self.message(),
                call,
                output,
                self.message("item_excluded", "Must not inherit later turn"),
            ]
        )
        reasoning = dict(
            type="response_item",
            payload=dict(
                type="reasoning",
                id="item_reason",
                encrypted_content="ENCRYPTED_KEEP",
                summary=[],
            ),
        )
        event = dict(
            ...
            payload=dict(
                ...
                thread_id=parent,
                item=dict(
                    type="Reasoning", id="item_reason", raw_content=["item_reason"]
                ),
            ),
        )
        child, child_path = self.add_thread(
            [reasoning, event],
            base=dict(thread_id=parent, end_ordinal_exclusive=4, end_byte_offset=1),
        )
        originals = {p: p.read_bytes() for p in (parent_path, child_path)}
        plan = repair.prepare(self.home, child)
        ...
        self.assertNotIn("history_base", meta)
        self.assertNotEqual(meta["id"], child)
        self.assertEqual(
            records[1]["payload"]["content"][0]["text"],
            "Retain item_old_message inside text unchanged",
        )
        self.assertEqual(
            records[2]["payload"]["arguments"], call["payload"]["arguments"]
        )
    ...
    def test_compaction_history_is_normalized_without_touching_text(self):
        ...
        compaction = dict(
            type="compacted",
            payload=dict(
                message="item_old_message",
                replacement_history=[message],
                guardian_history=[message],
                retained_context={"id": "item_old_message"},
            ),
        )
        tid, _ = self.add_thread([compaction])
    ...
    def test_no_change_creates_no_copy_and_invalid_uuid_fails(self):
        ...
        self.assertIn("没有发现", self.cli("repair-session", tid))
        self.assertFalse((self.m.root / "repairs").exists())
        self.cli("repair-session", "invalid", expected=1)

    def test_native_registered_or_archived_copy_is_not_duplicated(self):
        tid, _ = self.add_thread([self.message()])
        self.cli("repair-session", tid)
        manifest = json.loads(
            next((self.m.root / "repairs").glob("*.json")).read_text()
        )
        self.db.execute(
            "INSERT INTO threads SELECT ?,?,cwd,model_provider,title,name,source,thread_source,agent_path,0,updated_at,updated_at_ms FROM threads WHERE id=?",
            (manifest["id"], manifest["rollout_path"], tid),
        )
        self.db.commit()
        self.assertEqual(
            sum(x["id"] == manifest["id"] for x in self.m.sessions("codex")), 1
        )
        self.db.execute("UPDATE threads SET archived=1 WHERE id=?", (manifest["id"],))
        self.db.commit()
        self.assertNotIn(manifest["id"], [x["id"] for x in self.m.sessions("codex")])

    def test_changed_source_rejected(self):
        tid, path = self.add_thread([self.message()])
        plan = repair.prepare(self.home, tid)
    ...
    def test_missing_ancestor_cycle_boundary_and_symlink_rejected(self):
        tid, path = self.add_thread(
            [self.message()],
            base=dict(thread_id=str(uuid.uuid4()), end_ordinal_exclusive=3),
        )
        self.cli("repair-session", tid, expected=1)
        records = [json.loads(x) for x in path.read_text().splitlines()]
        records[0]["payload"]["history_base"]["thread_id"] = tid
            ...
            dict(
                ...
                payload=dict(
                    ...
                    item=dict(
                        type="UserMessage",
                        id="user_" + turn,
                        content=[dict(type="text", text=text, text_elements=[])],
                    ),
                ),
            ),
            self.message(),
    ...
    def old_copy(self):
        ...
        path.write_bytes(
            repair.serialize([x for x in records if "ai_switch_display" not in x])
        )
        manifest["version"] = 1
        manifest.pop("display_events_version")
        manifest.pop("display_events_added")
    ...
    def test_projection_deduplicates_per_turn_in_mixed_history(self):
        ...
        records.insert(
            ...
            dict(
                type="event_msg",
                payload=repair.legacy_message(records[1]["payload"]["item"]),
            ),
        )
    ...
    def test_projection_keeps_media_and_utf8_text_element_offsets(self):
        item = dict(
            type="UserMessage",
            content=[
                dict(type="text", text="你好"),
                dict(
                    type="text",
                    text="@file",
                    text_elements=[
                        dict(byte_range=dict(start=0, end=5), placeholder="@file")
                    ],
                ),
                dict(type="image", url="data:image/png;base64,AA"),
                dict(type="localImage", path="/tmp/image.png"),
                dict(type="audio", url="data:audio/wav;base64,AA"),
                dict(type="localAudio", path="/tmp/audio.wav"),
            ],
        )
        ...
        self.assertEqual(
            payload["text_elements"][0]["byte_range"], dict(start=7, end=12)
        )
    ...
    def test_upgrade_dry_run_keeps_same_uuid_appends_context_and_original(self):
        ...
        with path.open("ab") as out:
            out.write(
                repair.serialize(
                    self.modern_messages("later", "Appended after original repair.")
                )
            )
        before = {
            p: p.read_bytes()
        ...
        self.assertEqual(
            (self.home / "state_5.sqlite").read_bytes(),
            before[self.home / "state_5.sqlite"],
        )
        self.assertIn("消息显示记录已完整", self.cli("repair-session", tid))
        snapshot = json.loads(
            (Path(self.m.load()["last_backup"]) / "snapshot.json").read_text()
        )
        import base64

        self.assertEqual(
            base64.b64decode(
                next(x["data"] for x in snapshot if x["path"] == str(path))
            ),
            before[path],
        )
    ...
    def test_upgrade_failure_rolls_back_copy_and_manifest(self):
        ...
        real, failed = atomic_write, False
    ...
    def test_upgrade_detects_native_write_during_backup(self):
        ...
        before = {p: p.read_bytes() for p in (path, mp, self.m.state_path)}
        appended = (
            b'{"type":"event_msg","payload":{"type":"thread_settings_applied"}}\n'
        )
        real = atomic_write
    ...
    def repaired_native_fork(self, old_manifest=False):
        ...
        records = [
            json.loads(x)
            for x in Path(manifest["rollout_path"]).read_bytes().splitlines()
        ]
    ...
    def test_native_fork_regenerated_ids_removed_by_body_and_lineage(self):
        ...
        original_items = [
            i
            for r in map(json.loads, before[path].splitlines())
            for i in repair.response_items(r)
        ]
        repaired_items = [
            i
            for r in map(json.loads, plan["content"].splitlines())
            for i in repair.response_items(r)
        ]
        self.assertEqual(
            [{k: v for k, v in i.items() if k != "id"} for i in original_items],
            repaired_items,
        )
        self.assertIn(
            "由原生分支重新生成", self.cli("repair-session", tid, "--dry-run")
        )
        for p, raw in before.items():
            self.assertEqual(p.read_bytes(), raw)

    def test_old_manifest_fallback_and_multigeneration_fork(self):
        parent, parent_path, manifest, mp, _ = self.repaired_native_fork(
            old_manifest=True
        )
        ...
        with Path(manifest["rollout_path"]).open("ab") as f:
            f.write(b"{}\n")
        with self.assertRaises(repair.RepairError):
            repair.verify_sources(plan)
    ...
    def test_unrelated_session_valid_ids_and_changed_tool_results_are_kept(self):
        ...
        self.assertEqual(
            repair.prepare(self.home, unrelated, repair_dir=self.m.root / "repairs")[
                "changed"
            ],
            0,
        )
        for record in records:
            if record["type"] == "response_item":
                record["payload"]["content"][0]["text"] = "Different body"
        path.write_bytes(repair.serialize(records))
        self.assertEqual(
            repair.prepare(self.home, tid, repair_dir=self.m.root / "repairs")[
                "changed"
            ],
            0,
        )
    ...
    def test_saved_fingerprints_work_after_reference_rollout_removed(self):
        ...
        value["removed_response_fingerprints"] = ["bad"]
        mp.write_bytes(s.json_bytes(value))
        self.cli("repair-session", tid, expected=1)
    ...
    def test_fork_repair_keeps_call_ids_encrypted_reasoning_and_compaction(self):
        ...
        reasoning = dict(
            type="reasoning",
            id="item_reason",
            encrypted_content="ORIGINAL_ENCRYPTED",
            summary=[],
        )
        tid, _ = self.add_thread(
            [
                dict(
                    type="compacted",
                    payload=dict(
                        message="summary", replacement_history=[call, reasoning]
                    ),
                )
            ]
        )
        ...
        for i in rows[1]["payload"]["replacement_history"]:
            i["id"] = repair.PREFIXES[i["type"]] + "regenerated"
        fork, path = self.add_thread(rows[1:])
        rows = [json.loads(x) for x in path.read_bytes().splitlines()]
        rows[0]["payload"]["forked_from_id"] = manifest["id"]
        path.write_bytes(repair.serialize(rows))
        plan = repair.prepare(self.home, fork, repair_dir=self.m.root / "repairs")
        self.assertEqual(plan["changed"], 2)
        items = json.loads(plan["content"].splitlines()[1])["payload"][
            "replacement_history"
        ]
        self.assertEqual(items[0]["call_id"], "call_original")
        self.assertEqual(items[1]["encrypted_content"], "ORIGINAL_ENCRYPTED")
    ...
    def launch_fixture(
        ...
        url = gateway or (
            "https://aster.empeirion.cn:44444/v1"
            if mode == "aster"
            else "https://micu.example/v1"
        )
        profile = dict(
            values=dict(model="gpt-fixture", model_provider=mode),
            agents={},
            env={},
            providers={mode: dict(base_url=url)},
            launch_env={},
        )
        out = io.StringIO()
        with (
            patch.object(self.m.clients, "read_config", return_value=(None, {})),
            patch.object(self.m.profiles, "drift", return_value=([], False)),
            patch.object(self.m.profiles, "profile", return_value=profile),
            patch.object(self.m.switching, "use"),
            patch.object(
                self.m.diagnostics,
                "report",
                return_value=dict(apps={a: dict(matches_profile=True) for a in s.APPS}),
            ),
            patch.object(shutil, "which", return_value="/bin/" + app),
            patch.object(os, "chdir"),
            patch.object(platform_runtime, "launch", return_value=0) as execute,
    ...
    def test_auto_launch_repairs_original_and_selects_new_uuid(self):
        tid, path = self.add_thread(self.modern_messages())
        before = path.read_bytes()
        output, cmd = self.launch_fixture(tid)
        manifest = json.loads(
            next((self.m.root / "repairs").glob("*.json")).read_text()
        )
        self.assertNotEqual(manifest["id"], tid)
        self.assertEqual(cmd[-2:], ["resume", manifest["id"]])
        self.assertIn(manifest["id"], output)
    ...
    def test_auto_launch_reuses_pending_copy_and_preserves_continuation(self):
        ...
        _, cmd = self.launch_fixture(tid)
        copy_id = cmd[-1]
        mp = self.m.root / "repairs" / (copy_id + ".json")
        manifest = json.loads(mp.read_text())
        path = Path(manifest["rollout_path"])
        with path.open("ab") as f:
            f.write(
                repair.serialize(
                    [self.message("msg_native", "Later conversation remains")]
                )
            )
        before = path.read_bytes()
        output, cmd = self.launch_fixture(tid)
        self.assertEqual(cmd[-1], copy_id)
        self.assertIn("自动复用兼容副本", output)
        self.assertEqual(path.read_bytes(), before)
        self.assertEqual(len(list((self.m.root / "repairs").glob("*.json"))), 1)

    def test_auto_launch_clean_history_creates_no_copy(self):
        tid, path = self.add_thread([self.message("msg_real")])
        before = path.read_bytes()
        output, cmd = self.launch_fixture(tid)
    ...
    def test_auto_launch_changed_source_does_not_reuse_stale_branch(self):
        ...
        with path.open("ab") as f:
            f.write(repair.serialize([self.message("item_new", "New original turn")]))
        _, second = self.launch_fixture(tid)
        self.assertNotEqual(first[-1], second[-1])
        manifest = json.loads(
            (self.m.root / "repairs" / (second[-1] + ".json")).read_text()
        )
    ...
    def test_auto_launch_dry_run_writes_nothing(self):
        ...
        self.assertIn("以下副本 UUID 仅为预览", output)
        self.assertEqual(path.read_bytes(), original)
        self.assertEqual(self.m.state_path.read_bytes(), state)
        self.assertFalse((self.m.root / "repairs").exists())
        self.assertFalse((self.m.root / "backups").exists())
    ...
    def test_auto_launch_micu_claude_new_session_and_opt_out_bypass(self):
        ...
        for kwargs in (
            dict(mode="micu"),
            dict(app="claude"),
            dict(no_auto_repair=True),
        ):
            with (
                self.subTest(kwargs=kwargs),
                patch.object(
                    self.m.session_service,
                    "repair_session",
                    side_effect=AssertionError("must bypass"),
                ),
            ):
    ...
    def test_auto_launch_custom_aster_profile_and_other_gateway(self):
        ...
        _, cmd = self.launch_fixture(
            tid, mode="backup", gateway="https://aster.empeirion.cn:44444/v1"
        )
        self.assertNotEqual(cmd[-1], tid)

    def test_auto_launch_native_fork_regenerated_ids(self):
        tid, path, _, _, _ = self.repaired_native_fork()
        before = path.read_bytes()
        output, cmd = self.launch_fixture(tid)
        self.assertNotEqual(cmd[-1], tid)
        self.assertIn("1 个由原生分支重新生成", output)
        self.assertEqual(path.read_bytes(), before)

    def test_auto_launch_existing_copy_new_bad_ids_and_reuse_chain(self):
        original, _ = self.add_thread(self.modern_messages())
        _, cmd = self.launch_fixture(original)
        first = cmd[-1]
        m = json.loads((self.m.root / "repairs" / (first + ".json")).read_text())
        path = Path(m["rollout_path"])
        with path.open("ab") as f:
            f.write(repair.serialize([self.message("item_later", "Later bad API ID")]))
        before = path.read_bytes()
        _, cmd = self.launch_fixture(first)
        second = cmd[-1]
        self.assertNotEqual(first, second)
        self.assertEqual(path.read_bytes(), before)
        _, cmd = self.launch_fixture(original)
        self.assertEqual(cmd[-1], second)

    def test_auto_launch_deleted_copy_is_recreated(self):
        tid, _ = self.add_thread(self.modern_messages())
        _, cmd = self.launch_fixture(tid)
        first = cmd[-1]
        manifest = json.loads((self.m.root / "repairs" / (first + ".json")).read_text())
        Path(manifest["rollout_path"]).unlink()
        _, cmd = self.launch_fixture(tid)
        self.assertNotEqual(cmd[-1], first)

    def test_auto_launch_repair_failure_stops_client_and_preserves_source(self):
        tid, path = self.add_thread(self.modern_messages())
        before = path.read_bytes()
        with patch.object(
            self.m.storage, "transaction", side_effect=OSError("fixture failure")
        ):
            with self.assertRaises(OSError):
                self.launch_fixture(tid)
        self.assertEqual(path.read_bytes(), before)
        self.assertFalse((self.m.root / "repairs").exists())
    ...
    def test_auto_launch_rejects_override_before_repair(self):
        ...
        with patch.object(
            ...
            with self.assertRaises(s.SwitchError):
                self.launch_fixture(tid, client_args=["--model", "other"])
        self.assertFalse((self.m.root / "repairs").exists())
    ...
    def test_takeover_cli_flag_is_parsed_but_not_passed_to_codex(self):
        ...
        with patch.object(
            session_process, "ensure_available", return_value=False
        ) as guard:
    ...
    def test_takeover_requires_codex_and_uuid_before_any_process_check(self):
        with patch.object(
            session_process,
            "ensure_available",
            side_effect=AssertionError("no process check"),
        ):
            for kwargs in (
                dict(session=None),
                dict(session=str(uuid.uuid4()), app="claude"),
                dict(session="bad"),
            ):
                with self.assertRaises(s.SwitchError):
                    self.launch_fixture(takeover=True, **kwargs)
    ...
    def test_busy_session_stops_before_history_repair(self):
        tid, _ = self.add_thread(self.modern_messages())
        with patch.object(
            session_process, "ensure_available", side_effect=TakeoverError("busy")
        ):
            with patch.object(
                self.m.session_service,
                "repair_session",
    ...
    def test_takeover_preview_does_not_repair_actively_written_history(self):
        tid, _ = self.add_thread(self.modern_messages())
        with patch.object(
            session_process, "ensure_available", return_value=True
        ) as guard:
            with patch.object(
                self.m.session_service,
                "repair_session",
    ...
    def test_native_history_version_detection_and_failure(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "ai_switch.sessions.repair_version_probe", repair.__file__
        )
        ...
        for output, expected in [
            ("codex-cli 0.155.1", "legacy"),
            ("codex-cli 0.156.0", "paginated"),
            ("codex-cli 0.156.1", "paginated"),
            ("codex 1.0.0", "paginated"),
            ("unknown", "legacy"),
        ]:
            with patch.object(
                module.subprocess, "run", return_value=argparse.Namespace(stdout=output)
            ):
                self.assertEqual(module.native_history_mode(), expected)
        with patch.object(
            module.subprocess, "run", side_effect=subprocess.TimeoutExpired("codex", 10)
        ):
    ...
    def test_paginated_migration_new_uuid_preserves_context_and_source(self):
        ...
        with patch.object(repair, "native_history_mode", return_value="paginated"):
            ...
            manifest = json.loads(
                (self.m.root / "repairs" / (new_id + ".json")).read_text()
            )
            rows = list(
                map(
                    json.loads, Path(manifest["rollout_path"]).read_bytes().splitlines()
                )
            )
            self.assertEqual(rows[0]["payload"]["history_mode"], "paginated")
            self.assertEqual([r["ordinal"] for r in rows], list(range(len(rows))))
            for p in (path, mp, original, self.home / "state_5.sqlite"):
                self.assertEqual(p.read_bytes(), before[p])
            source_items = [
                i
                for r in map(json.loads, before[path].splitlines())
            ...
            self.assertEqual(
                [i for r in rows for i in repair.response_items(r)], source_items
            )
            self.assertIn(
                b"Recently appended message",
                Path(manifest["rollout_path"]).read_bytes(),
            )
    ...
    def test_paginated_projection_preserves_media_spans_and_deduplicates(self):
        user = dict(
            type="user_message",
            message="你好 @file",
            text_elements=[dict(byte_range=dict(start=7, end=12), placeholder="@file")],
        ...
        records = [
            dict(
                type="event_msg", payload=dict(type="task_started", turn_id="turn_one")
            ),
        ...
        projected = next(
            r["payload"]["item"] for r in rows if r.get("ai_switch_paginated")
        )
        ...
        second = [
            dict(
                type="event_msg", payload=dict(type="task_started", turn_id="turn_two")
            ),
            records[1],
        ]
    ...
    def test_paginated_projection_orphan_message_fails_closed(self):
        with self.assertRaises(repair.RepairError):
            repair.add_paginated_messages(
                [
                    dict(
                        type="event_msg",
                        payload=dict(type="user_message", message="orphan"),
                    )
                ],
                "thread",
            )
    ...
    def test_paginated_id_repair_flattens_fork_and_keeps_modern_events(self):
        ...
        child, path = self.add_thread(
            [self.message("item_later")],
            base=dict(thread_id=parent, end_ordinal_exclusive=5),
        )
        before = path.read_bytes()
    ...
    def test_format_migration_carries_id_fingerprints_for_later_forks(self):
        ...
        plan = repair.prepare(
            self.home,
            tid,
            repair_dir=self.m.root / "repairs",
            source_path=path,
            history_mode="paginated",
        )
        ...
        self.assertEqual(
            plan["manifest"]["removed_response_fingerprints"],
            old["removed_response_fingerprints"],
        )

    def test_paginated_clean_history_remains_unchanged(self):
        tid, path = self.add_thread([self.message("msg_valid")])
        before = path.read_bytes()
        with patch.object(repair, "native_history_mode", return_value="paginated"):
            _, command = self.launch_fixture(tid)
    ...
    def test_same_uuid_revert_segments_preserve_selected_prefix(self):
        tid, original = self.add_thread(
            [
                self.message(text="Keep early"),
                self.message("item_drop", "Dropped by revert"),
            ]
        )
        latest = self.add_segment(
            tid, original, 2, [self.message("item_new", "Keep revised")]
        )
        before = {
            p: p.read_bytes() for p in (original, latest, self.home / "state_5.sqlite")
        }
        plan = repair.prepare(
            self.home, tid, history_mode="paginated", repair_dir=self.m.root / "repairs"
        )
        rows = list(map(json.loads, plan["content"].splitlines()))
        self.assertEqual(
            [
    ...
    def test_multiple_reverts_resolve_by_byte_boundary_not_latest_database_row(self):
        tid, root = self.add_thread(
            [self.message(text="A"), self.message("item_old", "Old longer turn")]
        )
    ...
    def test_earlier_revert_and_cross_thread_fork_use_original_prefix(self):
        tid, root = self.add_thread(
            [
                self.message(text="A"),
                self.message("item_b", "B"),
                self.message("item_old", "Wrong old tail"),
            ]
        )
        one = self.add_segment(
            tid, root, 3, [self.message("item_later", "Wrong newer tail")]
        )
        reverted = self.add_segment(
            tid, root, 2, [self.message("item_revised", "Revised")]
        )
        # A different thread forked before the parent's later revert.
        fork, _ = self.add_thread(
            [self.message("item_fork", "Fork")],
            base=dict(
                thread_id=repair.rollout_segment_id(one),
                end_ordinal_exclusive=5,
                end_byte_offset=one.stat().st_size,
            ),
        )
        plan = repair.prepare(self.home, fork)
        self.assertIn(b"Wrong newer tail", plan["content"])
        self.assertNotIn(b"Revised", plan["content"])
        plan = repair.prepare(self.home, tid)
        self.assertIn(b"Revised", plan["content"])
        self.assertNotIn(b"Wrong", plan["content"])

    def test_ambiguous_segment_boundary_is_rejected(self):
        tid, root = self.add_thread(
            [self.message(text="A"), self.message("item_tail", "Original tail")]
        )
        first = self.add_segment(
            tid, root, 2, [self.message("item_one", "Different branch one")]
        )
        second = self.add_segment(
            tid, root, 2, [self.message("item_two", "Another branch two")]
        )
        # Two conflicting files claim the same physical segment identity.
        duplicate = first.with_name(
            first.name.replace(
                repair.rollout_segment_id(first), repair.rollout_segment_id(second)
            )
        )
        duplicate = duplicate.with_name(
            duplicate.name.replace("T00-00-00", "T01-00-00")
        )
