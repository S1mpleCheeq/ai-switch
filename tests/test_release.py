...
class ReleaseTests(unittest.TestCase):
    ...
    def test_archive_excludes_runtime_state_and_is_reproducible(self):
        ...
        self.assertEqual(
            release.build(self.root, self.root / "dist")[0].read_bytes(), first
        )
        self.assertTrue(
            archive.with_name(archive.name + ".sha256")
            .read_text()
            .startswith(hashlib.sha256(first).hexdigest())
        )
        ...
        for platform in ("linux", "macos", "windows"):
            ...
            if platform == "windows":
                with zipfile.ZipFile(archive) as bundle:
                    content = {
                        name.split("/", 1)[1]: bundle.read(name)
                        for name in bundle.namelist()
                    }
            else:
                with tarfile.open(archive) as bundle:
                    content = {
                        m.name.split("/", 1)[1]: bundle.extractfile(m).read()
    ...
    def test_arbitrary_template_key_and_symlinks_are_rejected(self):
        path = self.root / "src/ai_switch/resources/templates/aster/claude.json"
        ...
        path.unlink()
        path.symlink_to(
            self.root / "src/ai_switch/resources/templates/aster/codex.json"
        )
