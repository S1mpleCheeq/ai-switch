...
from ai_switch.platforms import linux as process
...
class ProcessTests(unittest.TestCase):
        ...
        def identity(pid):
            ...
            if pid == child.pid:
                value.update(
                    ...
                    arguments=arguments or [b"codex", b"resume"],
                )
            return value
    ...
    def test_missing_and_stale_unlocked_file_are_not_busy_or_deleted(self):
        ...
        with patch.object(
            signal, "pidfd_send_signal", side_effect=AssertionError("no signal")
        ):
            self.assertFalse(
    ...
    def test_takeover_only_signals_target_and_waits_for_release(self):
        ...
        with (
            self.as_native(child),
            patch.object(
                signal, "pidfd_send_signal", wraps=signal.pidfd_send_signal
            ) as send,
        ):
            self.assertFalse(
                process.ensure_available(
        ...
        self.assertEqual(
            [call.args[1] for call in send.call_args_list], [signal.SIGTERM]
        )
    ...
    def test_multiple_sessions_in_same_process_are_rejected(self):
        other = self.path.with_name(str(uuid.uuid4()) + ".lock")
        child = self.holder([self.path, other])
    ...
    def test_changed_lock_owner_and_pid_identity_are_not_signalled(self):
        ...
        with (
            self.as_native(child),
            patch.object(process, "writer", side_effect=[held, changed]),
        ):
            with patch.object(
                signal, "pidfd_send_signal", side_effect=AssertionError("no signal")
            ):
        ...
        with patch.object(
            process,
            "process_identity",
            side_effect=[identity, dict(identity, start="different")],
        ):
    ...
    def test_self_takeover_is_rejected(self):
        ...
        with self.path.open("w") as file:
            ...
            identity["executable"] = "/fixture/codex"
    ...
    def test_non_linux_dispatches_to_native_backend(self):
        from ai_switch.platforms import portable
        from ai_switch.platforms import processes as dispatch

        with (
            patch.object(dispatch.sys, "platform", "darwin"),
            patch.object(portable, "ensure_available", return_value=False) as native,
        ):
            self.assertFalse(
                dispatch.ensure_available(self.home, self.session, takeover=True)
            )
