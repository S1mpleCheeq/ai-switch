...
from ai_switch.platforms import (
    runtime as platform_runtime,
    io as platform_io,
    processes as session_process,
)

parser = argparse.ArgumentParser(
    description="Exercise installed native clients against local fake APIs; no real credentials."
)
...
if args.catalog is None:
    ...
    model = dict(
        ...
        description="Synthetic integration test model",
        default_reasoning_level="high",
        supported_reasoning_levels=[
            dict(effort=e, description=e) for e in ("low", "medium", "high", "xhigh")
        ],
        shell_type="unified_exec",
        visibility="list",
        supported_in_api=True,
        priority=1,
        base_instructions="Reply briefly. Do not use tools.",
    ...
    args.catalog.write_text(
        json.dumps(
            dict(
                models=[
                    dict(model, slug=slug)
                    for slug in ("gpt-6-astra", "gemini-3.8-flash-high")
                ]
            )
        ),
        encoding="utf-8",
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        d = json.loads(raw or "{}")
        requests.append((self.path, d))
        if "/responses" in self.path and hold_next.is_set():
            hold_next.clear()
            hold_started.set()
            hold_release.wait(40)
            return
        self.send_response(200)
        ...
        if "/responses" in self.path:
            ...
            emit(
                "response.output_item.added",
                {
                    ...
                    "output_index": 0,
                    "item": dict(msg, status="in_progress", content=[]),
                },
            )
            emit(
...
for mode in switch.MODES:
    for app in switch.APPS:
        ...
        if app == "codex":
            profile["providers"][mode]["base_url"] = base + "/" + mode + "/v1"
        else:
            profile["env"]["ANTHROPIC_BASE_URL"] = base + "/" + mode
        platform_io.atomic_write(
            manager.root / "profiles" / mode / (app + ".json"),
            switch.json_bytes(profile),
        )
state = manager.load()
for app in switch.APPS:
    _, config = manager.read_config(state, app)
    platform_io.atomic_write(
        Path(state["paths"][app]),
        manager.render(app, config, manager.profile("micu", app), "micu"),
    )
...
def run(label, cmd, extra={}):
    ...
    try:
        ...
        raise RuntimeError(label + " timed out " + err[-1500:])
    (root / (label + ".out")).write_text(out)
    (root / (label + ".err")).write_text(err)
    print(
        label,
        "exit",
        p.returncode,
        "requests",
        [(path, d.get("model")) for path, d in requests],
        flush=True,
    )
    if p.returncode:
        raise RuntimeError(label + " failed: " + err[-2000:] + out[-1000:])
    return out
...
try:
    ...
    out = run(
        "codex-new",
        cmd + ["exec", "--skip-git-repo-check", "--json", "REMEMBER fixture-7"],
        child_env,
    )
    sid = next(
        json.loads(l)["thread_id"]
        for l in out.splitlines()
        if json.loads(l).get("type") == "thread.started"
    )
    for mode in ["micu", "aster", "backup-micu", "backup-aster"]:
        route = mode.removeprefix("backup-")
        before = len(requests)
        cmd, child_env = client_command("codex", mode, sid)
        # The interactive resume flags are identical; exec makes the fixture noninteractive.
        cmd.insert(cmd.index("resume"), "exec")
        out = run(
            "codex-" + mode,
            cmd + ["--skip-git-repo-check", "--json", "CONTINUE fixture-7"],
            child_env,
        )
        req = requests[before:]
        assert any("/" + route + "/" in p for p, d in req)
        ...
        assert all(
            d.get("reasoning", {}).get("effort") == "xhigh"
            for p, d in req
            if "/responses" in p
        )
        assert manager.profile(mode, "codex")["values"]["model_reasoning_effort"] == (
            "xhigh" if route == "micu" else "ultra"
        )
    ...
    child = subprocess.Popen(
        cmd + ["--skip-git-repo-check", "--json", "WAIT for local takeover fixture"],
        env=child_env,
        cwd=root,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ...
    try:
        ...
        assert session_process.ensure_available(
            root / "codex", sid, takeover=True, dry_run=True
        )
        assert child.poll() is None, "Dry run terminated the native client"
        assert not session_process.ensure_available(
            root / "codex", sid, takeover=True, timeout=15
        )
        ...
        if child.poll() is None:
            ...
            for item in psutil.Process(child.pid).children(recursive=True):
                item.kill()
            child.kill()
    ...
    before = len(requests)
    run(
        "codex-after-takeover",
        cmd + ["--skip-git-repo-check", "--json", "CONTINUE fixture-7 after takeover"],
        child_env,
    )
    ...
    for i, mode in enumerate(["aster", "micu", "aster", "backup-micu", "backup-aster"]):
        ...
        cmd += [
            ...
            "json",
            "-p",
            "REMEMBER fixture-9" if i == 0 else "CONTINUE fixture-9",
        ]
        if not i:
            cmd += ["--session-id", sid]
        before = len(requests)
        out = run("claude-" + str(i), cmd, child_env)
        req = [(p, d) for p, d in requests[before:] if "/messages" in p]
        assert any("/" + route + "/" in p for p, d in req)
        expected = "claude-fable-5-1" if route == "aster" else "claude-sonnet-5"
        assert all(d["model"] == expected for p, d in req)
        if i:
            assert any("REMEMBER fixture-9" in json.dumps(d) for p, d in req)
        if route == "micu":
