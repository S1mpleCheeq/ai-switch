...
class PlatformIOTests(unittest.TestCase):
    ...
    def test_permissions_are_private(self):
        ...
        advapi.GetNamedSecurityInfoW.argtypes = [w.LPWSTR, ctypes.c_int, w.DWORD, w.LPVOID, w.LPVOID, ctypes.POINTER(w.LPVOID), w.LPVOID, ctypes.POINTER(w.LPVOID)]
        ...
        self.assertEqual(advapi.GetNamedSecurityInfoW(str(path), 1, 4, None, None, ctypes.byref(acl), None, ctypes.byref(descriptor)), 0)
        try:
            ...
            for index in range(count):
                ...
                self.assertTrue(advapi.ConvertSidToStringSidW(ace.value + 8, ctypes.byref(value)))
                try:
