...
class OnboardingTests(unittest.TestCase):
    def setUp(self):
        ...
        self.ca.write_text(ssl.DER_cert_to_PEM_cert(ssl.create_default_context().get_ca_certs(binary_form=True)[0]))
        self.catalog = self.root/'catalog.json'
        self.catalog.write_text(json.dumps({'models': [{'slug': 'gpt-6-astra'}, {'slug': 'gemini-3.8-flash-high'}]}))
        self.m = s.Manager(self.root/'state')
        self.env = patch.dict(os.environ, {'ASTERGATE_API_KEY': 'ASTER_FIXTURE', 'MICU_FIXTURE': 'MICU_FIXTURE_VALUE'})
        self.env.start(); self.addCleanup(self.env.stop)

    def init(self, app, ask=False):
        self.selected = app
        self.live = self.root/app/('settings.json' if app == 'claude' else 'config.toml')
        if app == 'claude':
            self.live.write_text(json.dumps(dict(model='sonnet', env=dict(ANTHROPIC_BASE_URL='https://micu.example', ANTHROPIC_AUTH_TOKEN='MICU_FIXTURE_VALUE'))))
        else:
            self.live.write_text('model="gpt-6-astra"\nmodel_provider="micu"\n[model_providers.micu]\nname="Micu"\nbase_url="https://micu.example/v1"\nwire_api="responses"\nenv_key="MICU_FIXTURE"\n')
        self.original = self.live.read_bytes()
        self.other = self.root/('codex' if app == 'claude' else 'claude')/('config.toml' if app == 'claude' else 'settings.json')
        self.other.write_bytes(b'Unselected file must never be read or modified')
        args = argparse.Namespace(app=app, codex_dir=str(self.root/'codex'), claude_dir=str(self.root/'claude'),
                                  catalog=str(self.catalog) if app == 'codex' else None, ca=str(self.ca),
                                  aster_key_env='ASTERGATE_API_KEY', ask_api_key=ask)
    ...
    def lifecycle(self, app):
        ...
        self.cli('profile', 'edit', 'backup', '--app', app, '--model', 'fixture-model', '--ask-api-key', expected=1)
        ...
        self.assertEqual(self.live.read_bytes(), self.original)
        with patch.object(s.shutil, 'which', side_effect=lambda name: '/fixture/'+name if name == app else None):
            self.cli('check')
            self.cli('run', app, '--dry-run')
        self.assertEqual(set(self.m.report()['apps']), {app})
        self.assertEqual(self.other.read_bytes(), b'Unselected file must never be read or modified')
    ...
    def test_codex_only_full_lifecycle(self):
        ...
        self.assertEqual(self.m.profile('micu', 'codex')['launch_env']['MICU_FIXTURE'], 'MICU_FIXTURE_VALUE')
    ...
    def test_missing_client_commands_fail_clearly_without_changes(self):
        ...
        for command in [('use', 'aster', '--app', 'codex'), ('run', 'codex', '--dry-run'),
                        ('sessions', '--app', 'codex'), ('capture', '--app', 'codex'),
    ...
    def test_interactive_key_never_appears_in_output(self):
        with patch.object(s.sys.stdin, 'isatty', return_value=True), patch.object(s.getpass, 'getpass', return_value='ASKED_FIXTURE') as ask:
    ...
    def test_template_works_before_init_without_reading_state(self):
        with patch.object(s.Manager, 'load', side_effect=AssertionError('must not read state')):
            value = json.loads(self.cli('profile', 'template', 'aster', '--app', 'codex'))
        self.assertEqual(value['providers']['aster']['experimental_bearer_token'], '__ASTERGATE_API_KEY__')
    ...
    def test_codex_missing_catalog_fails_before_saving_state(self):
        ...
        args = argparse.Namespace(app='codex', codex_dir=str(self.root/'codex'), claude_dir=str(self.root/'claude'),
                                  catalog=None, ca=str(self.ca), aster_key_env='ASTERGATE_API_KEY')
