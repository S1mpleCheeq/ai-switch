...
def native_history_mode():
    ...
    try:
        result = subprocess.run(
            platform_runtime.native_command(
...
def legacy_message(item):
    ...
    return dict(
        ...
        local_images=[
            p["path"] for p in parts if p.get("type") in ("local_image", "localImage")
        ],
        audio=[p["url"] for p in parts if p.get("type") == "audio"],
        local_audio=[
            p["path"] for p in parts if p.get("type") in ("local_audio", "localAudio")
        ],
    )
    ...
    for record in records:
        ...
        result.append(
            dict(
                ...
                payload=message,
                ai_switch_display=dict(
                    version=1, turn_id=key[0], item_id=payload.get("item", {}).get("id")
                ),
            )
        )
...
def add_paginated_messages(records, thread_id):
    ...
    def key(turn, message):
        ...
        value = {
            k: (message.get(k) or [])
            if k not in ("type", "message", "phase")
            else message.get(k)
            for k in fields
        }
    ...
    for record in records:
        ...
        if not event_turn:
            raise RepairError(
                "旧消息缺少所属轮次，无法安全转换分页历史；未修改源会话。"
            )
        user = kind == "user_message"
        item = dict(
            type="UserMessage" if user else "AgentMessage",
            id=str(uuid.uuid4()),
            content=[dict(type="text" if user else "Text", text=payload["message"])],
        )
        if user:
            item["content"][0]["text_elements"] = copy.deepcopy(
                payload.get("text_elements", [])
            )
            for field, media_type, value_key in (
                ("images", "image", "url"),
...
def prepare_display_upgrade(codex_home, session_id, manifest):
    ...
    if (
        ...
        or not any(
            path.resolve().is_relative_to(home / name)
    ...
    if (
        ...
        or meta.get("history_base")
        or meta.get("history_mode") != "legacy"
    ):
    ...
    updated.update(
        version=2,
        display_events_version=1,
        records=len(records),
        rollout_sha256=hashlib.sha256(content).hexdigest(),
    )
    updated["display_events_added"] = dict(
        Counter(updated.get("display_events_added", {})) + Counter(added)
    )
    return dict(
        source_id=session_id,
        changed=sum(added.values()),
        upgrade=True,
        path=path,
        content=content,
        manifest=updated,
...
def is_subagent(source, thread_source=None, agent_path=None):
    ...
    return bool(
        ...
        or source == "subagent"
        or thread_source in ("subagent", "guardian_review")
        or agent_path
...
def checked_rollout_path(home, path):
    ...
    path = path.resolve()
    if not any(
        path.is_relative_to(home / name) for name in ("sessions", "archived_sessions")
    ):
...
def resolve_base_path(home, conn, base, child_path):
    ...
    paths = set()
    row = conn.execute(
        "SELECT rollout_path FROM threads WHERE id=?", (ident,)
    ).fetchone()
    ...
    if len({c[2] for c in candidates}) != 1:
        raise RepairError(
            f"祖先分段存在多个不同候选，无法安全确定历史：{ident}；未修改会话。"
        )
...
def repaired_ancestor_fingerprints(home, session_id, repair_dir):
    ...
    seen, pending, fingerprints, sources, references = (
        set(),
        [(session_id, None)],
        set(),
        [],
    ...
    try:
        ...
        while pending:
            ...
            if manifest_path.exists():
                ...
                if manifest.get("id") != ident or manifest.get("version") not in (
                    1,
                    2,
                    3,
                ):
            ...
            if header.get("type") != "session_meta" or (
                meta.get("id") != ident
                and (explicit_path is None or rollout_segment_id(path) != ident)
            ):
                raise RepairError("修复祖先的会话 UUID 不一致。")
            if (
                meta.get("id") != ident
                and (repair_dir / (meta["id"] + ".json")).is_file()
            ):
            ...
            if meta.get("history_base"):
                ...
                pending.append(
                    (base["thread_id"], resolve_base_path(home, conn, base, path))
                )
    finally:
        conn.close()
...
def prepare(
    codex_home, session_id, repair_dir=None, source_path=None, history_mode="legacy"
):
    ...
    try:
        ...
        while True:
            ...
            header = json.loads(lines[0])
            start_ordinal = (header.get("payload", {}).get("history_base") or {}).get(
            ...
            if not raw.endswith(b"\n"):
                raise RepairError(
                    "会话仍在写入或历史截断位置不是完整记录边界，请退出该会话后重试。"
                )
            records = [json.loads(line) for line in raw.splitlines()]
            if not records or records[0].get("type") != "session_meta":
                raise RepairError("无法识别会话元数据。")
            meta = records[0]["payload"]
            if meta.get("id") != ident and (
                explicit_path is None or rollout_segment_id(path) != ident
            ):
            ...
            if (
                ...
                or not base.get("thread_id")
                or "end_ordinal_exclusive" not in base
            ):
                raise RepairError("无法识别分页祖先引用，未创建修复副本。")
            ...
            ident, limit = (
                str(uuid.UUID(base["thread_id"])),
                base["end_ordinal_exclusive"],
            )
            explicit_path = resolve_base_path(home, conn, base, path)
    finally:
        conn.close()
    fingerprints, reference_nodes, reference_ids = repaired_ancestor_fingerprints(
        home, session_id, repair_dir
    )
    ...
    for record in records[1:]:
        for item in response_items(record):
            ...
            if (
                isinstance(old, str)
                and kind in PREFIXES
                and (old.startswith("item_") or signature in fingerprints)
            ):
                ...
                if not old.startswith("item_"):
                    regenerated.add(old)
    source_nodes = [
        {k: v for k, v in n.items() if k != "records"} for n in nodes
    ] + reference_nodes
    migrate = (
        history_mode == "paginated"
        and metadata["payload"].get("history_mode") != "paginated"
    )
    ...
    for record in records[1:]:
        ...
        if (
            ...
            and payload.get("thread_id") in visited_ids
        ):
    ...
    filename = (
        "rollout-" + created.strftime("%Y-%m-%dT%H-%M-%S") + "-" + new_id + ".jsonl"
    )
    path = home / "sessions" / created.strftime("%Y/%m/%d") / filename
    counts = {
        kind: list(kinds.values()).count(kind) for kind in sorted(set(kinds.values()))
    }
    manifest = dict(
        version=3 if history_mode == "paginated" else 2,
        history_mode=history_mode,
        format_migrated=migrate,
        paginated_events_added=paginated_added,
        display_events_version=1,
        display_events_added=display_added,
        regenerated_ids=len(regenerated),
        repair_reference_ids=reference_ids,
        removed_response_fingerprints=sorted(removed_fingerprints | fingerprints),
        source_id=session_id,
        id=new_id,
        cwd=meta.get("cwd", source_row["cwd"]),
        provider=meta.get("model_provider", source_row["model_provider"]),
        name=("[兼容副本] " if migrate else "[ID 修复] ")
        + (source_row.get("name") or source_row.get("title") or session_id),
        created_at_ms=int(created.timestamp() * 1000),
        rollout_path=str(path),
        rollout_sha256=hashlib.sha256(raw).hexdigest(),
        changed=len(mapping),
        changed_types=counts,
        records=len(records),
        source_nodes=source_nodes,
    )
    manifest["subagent"] = is_subagent(
        source_row.get("source"),
        source_row.get("thread_source"),
        source_row.get("agent_path"),
    )
    return dict(
        source_id=session_id,
        changed=len(mapping),
