...
def windows_file_users(path):
    ...
    rm.RmRegisterResources.argtypes = [
        w.DWORD,
        w.UINT,
        ctypes.POINTER(w.LPCWSTR),
        w.UINT,
        ctypes.POINTER(Unique),
        w.UINT,
        ctypes.POINTER(w.LPCWSTR),
    ]
...
def validate_owner(held, info, home):
    ...
    if info["user"] != current.username() or Path(
        info["executable"]
    ).name.lower() not in ("codex", "codex.exe"):
        raise TakeoverError("占用者不是当前用户的原生 Codex，未结束进程。")
    if any(
        arg in ("app-server", "exec-server", "daemon", "remote-control")
        for arg in info["arguments"][1:]
    ):
    ...
    if os.name == "nt":
        ...
        configured = env.get("CODEX_HOME") or str(
            Path(env.get("USERPROFILE", str(Path.home()))) / ".codex"
        )
        ...
        for path in paths:
            if (
                path.resolve() != held.path
                and busy(path) is not None
                and held.pid in file_users(path)
            ):
        ...
        for opened in process.open_files():
            ...
            if (
                path.parent.name == "thread-writer-locks"
                and path.resolve() != held.path
            ):
                try:
                    uuid.UUID(path.stem)
                except ValueError:
...
def termination_handle(info, dry_run):
    ...
    handle = kernel.OpenProcess(
        0x100000 | 0x1000 | (0 if dry_run else 1), False, info["pid"]
    )
    if not handle:
    ...
    try:
        ...
        if abs(started - info["started"]) > 0.001:
...
def ensure_available(
    home, session, *, takeover=False, dry_run=False, timeout=15.0, emit=print
):
    import psutil
    ...
    try:
        ...
        if not takeover:
            raise TakeoverError(
                f"会话 {session} 仍被其他进程占用；可使用 --takeover（先加 --dry-run 预览）。"
            )
        if held.pid is None:
        ...
        with termination_handle(info, dry_run) as handle:
            ...
            action = (
                "终止单个 Windows 进程（未保存工作可能丢失）"
                if os.name == "nt"
                else "发送 SIGTERM"
            )
            if dry_run:
                emit(
                    f"仅预览：将向会话 {session} 的 PID {held.pid} {action}；未结束进程。"
                )
                return True
