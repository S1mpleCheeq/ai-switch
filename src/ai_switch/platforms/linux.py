"""Linux Codex writer-lock takeover via /proc and kernel pidfd."""

from __future__ import annotations

try:
    import fcntl
except ImportError:  # Allow test discovery on systems that skip Linux cases.
    fcntl = None
import os
...
def locked_descriptors(pid):
    ...
    for descriptor in descriptors:
        try:
            lines = (
                Path(f"/proc/{pid}/fdinfo/{descriptor.name}").read_text().splitlines()
            )
            locks = parse_locks(line for line in lines if line.startswith("lock:"))
            if any(owner == pid for owner, _ in locks):
                result.append(
                    (file_key(descriptor.stat()), Path(os.readlink(descriptor)))
                )
        except (FileNotFoundError, PermissionError):
            continue
    return result
...
def process_identity(pid):
    ...
    arguments = (proc / "cmdline").read_bytes().split(bytes([0]))
    return dict(
        start=fields[19],
        ppid=int(fields[1]),
...
def validate_owner(held, identity):
    ...
    if identity["uid"] != os.getuid() or Path(identity["executable"]).name != "codex":
        raise TakeoverError(
            f"PID {held.pid} 不是当前用户的原生 Codex 进程，未结束进程。"
        )
    if any(
        arg in (b"app-server", b"exec-server", b"daemon", b"remote-control")
        for arg in identity["arguments"][1:]
    ):
        raise TakeoverError(
            f"PID {held.pid} 是后台服务，不能按单个会话结束它；未结束进程。"
        )
    ancestor = os.getpid()
    seen = set()
    while ancestor > 0 and ancestor not in seen:
        if ancestor == held.pid:
            raise TakeoverError(
                "目标进程是当前命令自身或其父进程，不能在该会话内部接管自己；请在另一个终端执行。"
            )
        seen.add(ancestor)
        try:
            fields = (
                Path(f"/proc/{ancestor}/stat").read_text().rsplit(")", 1)[1].split()
            )
...
def ensure_available(
    ...
    if not takeover:
        raise TakeoverError(
            f"会话 {session} 正被 {owner} 占用；如需结束旧进程并续接，请加 --takeover（可先加 --dry-run 预览）。"
        )
    if held.pid is None:
    ...
    try:
        ...
        if dry_run:
            emit(
                f"仅预览：将向会话 {session} 的 PID {held.pid} 发送 SIGTERM，等待退出及写锁释放；未发送信号。"
            )
            return True
        emit(
            f"接管会话 {session}：向 PID {held.pid} 发送 SIGTERM，等待退出及写锁释放（最多 {timeout:g} 秒）。"
        )
        ...
        while True:
            ...
            if current is not None and (
                current.key != held.key or current.pid not in (None, held.pid)
            ):
                raise TakeoverError(
                    "等待期间会话被其他进程占用，未结束新占用者；未启动客户端。"
                )
            if time.monotonic() >= deadline:
                raise TakeoverError(
                    f"等待 PID {held.pid} 退出及释放写锁超时；未执行 SIGKILL，未启动客户端。"
                )
