...
def protect_file(path, mode=0o600):
    ...
    descriptor = w.LPVOID()
    sddl = "D:P" + "".join(
        f"(A;{inherit};FA;;;{sid})" for sid in (windows_sid(), "SY", "BA")
    )
...
def file_lock(fd, acquire=True):
    ...
    ok = (
        kernel.LockFileEx(handle, 3, 0, 0xFFFFFFFF, 0xFFFFFFFF, ctypes.byref(overlap))
        if acquire
        else kernel.UnlockFileEx(
            handle, 0, 0xFFFFFFFF, 0xFFFFFFFF, ctypes.byref(overlap)
        )
    )
    if not ok:
